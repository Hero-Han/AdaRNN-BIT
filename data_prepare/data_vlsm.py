...
def generate_samples(x, y, model_params, seq_len_before=9, seq_len_after=9, output_seq_len=6):
    """
    ...
    z = np.zeros((output_seq_len, model_params['dim_in']))##(6, 16)
...
def pad_all_cases(x, y, model_params, min_len_before=9, max_len_before=9, min_len_after=9, max_len_after=9,
                  targetlength=6):
    """
    ...
    for l_before in range(min_len_before, max_len_before + 1):
        for l_after in range(min_len_after, max_len_after + 1):
            case_x, case_y = generate_samples(x, y, model_params, l_before, l_after, targetlength)
            # npad is a tuple of (n_before, n_after) for each dimension

            len_x = np.full(case_x.shape[0], case_x.shape[1])##创建数组，第一个表示的数组的形状第二个表示的是填充的数字是啥

            len_before_sequence_x = np.full(case_x.shape[0], l_before)##创建数组，第一个表示的是分割出来的训练数据的个数，填充的是before数
