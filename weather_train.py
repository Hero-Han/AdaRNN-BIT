...
from model_king.Dual_AdaRNN import Dual_AdaRNN, Cross_Attention, Decoder, Share_Encoder
import weather_data.TEM_process as data_process
...
def get_model(name='DualAdaRNN'):
    ...
    share_encoder = Share_Encoder(n_input=args.d_feat, n_hiddens=n_hiddens, dec_layers=args.dec_layers,
                                  dropout=args.dropout, model_type=name,
                                  len_seq=args.len_seq, trans_loss=args.loss_type)
    cross_attention = Cross_Attention(n_hiddens=n_hiddens)
    decoder = Decoder(output_dim=args.class_num, n_hiddens=n_hiddens, dec_layers=args.dec_layers, dropout=args.dropout,
                      attention=cross_attention)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    return Dual_AdaRNN(share_encoder=share_encoder, decoder=decoder, output_dim=args.class_num, len_seq=args.len_seq,
                       device=device).to(device)


def train_DualRNN(args, model_king, optimizer, train_loader_list, epoch, teacher_forcing_ratio, alpha, beta,
                  dist_old_before=None, dist_old_after=None, weight_mat_before=None, weight_mat_after=None):
    model_king.train()
    criterion = nn.MSELoss()  ##代价函数
    criterion_1 = nn.L1Loss()  ##代价函数
    loss_all = []
    loss_1_all = []
    dist_mat_before = torch.zeros(args.num_layers, args.len_seq).to(device)  ##[两行，10列]
        ...
        for i in range(len(index)):
            ...
            if epoch < args.pre_epoch:
                ...
                pred_s, pred_t, decoder_att, loss_transfer, dist_before, dist_after, weight_mat_before, weight_mat_after = model_king.for_Boosting(
                    feature_all_left, feature_all_right, ytrue_all, teacher_forcing_ratio, weight_mat_before,
                    weight_mat_after)
...
def train_epoch_transfer_Boosting(model, optimizer, train_loader_list, epoch, teacher_forcing_ratio, alpha, beta,
                                  dist_old_before=None, dist_old_after=None, weight_mat_before=None,
...
def test_epoch(model, test_loader, teacher_forcing_ratio, batch, N_output, prefix='Test'):
    ...
    for fea_left, fea_right, y_truth in tqdm(test_loader, desc=prefix, total=len(test_loader)):
        ...
        loss_dtw = loss_dtw / batch
        loss_tdi = loss_tdi / batch
        ...
        # show_attention(fea_left_tensor, fea_right_tensor, out, atten)
...
def test_epoch_inference(model, test_loader, batch, teacher_forcing_ratio, prefix='Test'):
    model.eval()
    total_loss = 0
    total_loss_1 = 0
    total_loss_r = 0
    ...
    loss_1 = total_loss_1 / len(test_loader)
    loss_r = total_loss_r / len(test_loader)
    return loss, loss_1, loss_r, y_list, predict_list


def inference(model, data_loader, batch, teacher_forcing_ratio):
    loss, loss_1, loss_r, y_list, predict_list = test_epoch_inference(
        model, data_loader, batch, teacher_forcing_ratio, prefix='Inference')
    return loss, loss_1, loss_r, y_list, predict_list


def inference_all(output_path, model, model_path, loaders, batch, teacher_forcing_ratio):
    pprint('inference...')
    loss_list = []
    loss_l1_list = []
    loss_r_list = []
    ...
    for loader in loaders:
        loss, loss_1, loss_r, label_list, predict_list = inference(
            model, loader, batch, teacher_forcing_ratio)
...
def transform_type(init_weight):
    weight = torch.ones(args.num_layers, args.len_seq).to(device)  ##2行10列
...
def main_transfer(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.model_name in ['Boosting']:
            loss, loss1, weight_mat_before, weight_mat_after, dist_mat_before, dist_mat_after = train_epoch_transfer_Boosting(
                model, optimizer, train_loader_list, epoch, dist_mat_before, dist_mat_after, weight_mat_before,
                weight_mat_after)
            ...
            loss, loss1, weight_mat_before, weight_mat_after, dist_mat_before, dist_mat_after = train_DualRNN(
                args, model, optimizer, train_loader_list, epoch, teacher_forcing_ratio, alpha, beta, dist_mat_before,
                dist_mat_after, weight_mat_before, weight_mat_after)
        else:
    ...
    loss_list, loss_l1_list, loss_r_list = inference_all(output_path, model, os.path.join(
        output_path, save_model_name), loaders, batch, teacher_forcing_ratio)
...
def get_args():
    ...
    parser.add_argument('--batch_size', type=int, default=36)  ##batch_size是分批
    parser.add_argument('--dw', type=float, default=0.1)  # 0.05, 1.0, 5.0, 0.05
    ...
    parser.add_argument('--num_domain', type=int, default=5)
    ...
    parser.add_argument('--data_path', default="/Volumes/王九和/科研/农业大数据相关/实验/实验程序/玉皇/AdaRNN-BIT/weather_data")
