...
class Share_Encoder(nn.Module):
    ...
    def forward_pre_train(self, input_before, input_after, len_win=0):
        ...
        outputs_before, hidden_before, out_lis_before, out_weight_list_before, hidden_lis_before = self.gru_features_left(input_l)
        ...
        outputs_after, hidden_after, out_lis_after, out_weight_list_after, hidden_lis_after = self.gru_features_right(input_r)
        ...
        return outputs_before, outputs_after,out_lis_before, out_lis_after,hidden_lis_before, hidden_lis_after, out_weight_list_before, \
               out_weight_list_after, hidden_decoder_l,hidden_decoder, loss_transfer

    def predict(self, input_before, input_after):
        input_bp = self.input_linear(input_before)
        outputs_before, hidden_before, out_lis_before, out_weight_list_before, hidden_lis_before = self.gru_features_left(input_bp,predict=True)
        hidden_before = torch.tanh(
            self.output_linear_left(
                torch.cat((hidden_before[-2, :, :], hidden_before[-1, :, :]),
                          dim=1)))
        ...
        outputs_after, hidden_after, out_lis_after, out_weight_list_after,hidden_lis_after = self.gru_features_right(input_ap,predict=True)

        hidden_decoder_l = hidden_before.repeat(self.dec_layers, 1, 1)  ##(1,2,64)
        hidden_decoder_r = hidden_after.repeat(self.dec_layers, 1, 1)
        hidden_decoder = torch.cat((hidden_decoder_l, hidden_decoder_r), dim=0)

        return outputs_before, outputs_after,out_lis_before, out_lis_after,hidden_lis_before, hidden_lis_after, hidden_decoder_l, hidden_decoder
    ...
    def forward_boosting(self, input_before, input_after, weight_mat_before=None, weight_mat_after=None):
        ...
        loss_transfer = loss_transfer_before + loss_transfer_after
        return outputs_before, outputs_after, out_lis_before, out_lis_after, hidden_lis_before, hidden_lis_after,\
               weight_before, weight_after, hidden_decoder_l,hidden_decoder, loss_transfer, dist_mat_before, dist_mat_after
    ...
    def gru_features_left(self, x, predict=False):
        ...
        return out, hidden, out_lis, out_weight_list,hidden_lis
...
class Cross_Attention(nn.Module):
    ...
    def forward_or(self, hidden, encoder_outputs):
    ...
    def forward_new(self,hiddenlis_l, hiddenlis_r, outlis_l, outlis_r):
        m = outlis_r[0]
        f = len(outlis_l)
        len_seq = m.shape[1]
        batch = m.shape[0]
        atten = torch.zeros(f, batch, len_seq * 2).to(device)
        for i in range(len(outlis_l)):
            out_l = outlis_l[i]
            out_l = out_l.permute(1, 0, 2)  ##(10,3,128)
            out_r = outlis_r[i]
            out_r = out_r.permute(1, 0, 2)  ##(10,3,128
            encoupts = torch.cat((out_l, out_r), dim=0)
            hidden_l = hiddenlis_l[i]
            hidden_r = hiddenlis_r[i]  ##(2,3,64)
            enhidden = torch.cat((hidden_l, hidden_r), dim=0)
            batch_size = encoupts.shape[1]
            src_len = encoupts.shape[0]
            hidden = torch.unbind(enhidden, dim=0)[0]  ##只取最近层的hidden信息,是按照维度做切片,(2,64)
            hidden = hidden.unsqueeze(1).repeat(1, src_len, 1)  ##3,20,64
            encoder_outputs = encoupts.permute(1, 0, 2)  ##3,20,128
            energy = torch.tanh(
                self.attn(torch.cat((hidden, encoder_outputs),
                                    dim=2)))  ##主要操作是将hidden的一维向量重复24次作为上一阶段的状态s，然后与h粘贴生成（(2,24,64*3)通过全联接生成(2,24,64)
            energy = energy.permute(0, 2, 1)  ##选取20行的各个特征维度进行重构形成(3,64,24)
            v = self.v.repeat(batch_size, 1).unsqueeze(1)  ##3,1,64
            attention = torch.bmm(v, energy).squeeze(1)  ##生成（b,n,d）(3,20)##类似于outweight的权重，boosting可以用来调整这个
            atten[i] = attention.unsqueeze(0)
        attentions = torch.mean(atten, dim=0)
        return F.softmax(attentions, dim=1)


class Decoder(nn.Module):
    def __init__(self, output_dim, n_hiddens, dec_layers, dropout, attention,attention_type = 'org'):  ##1,60,60,1
    ...
    def forward(self, input, hidden,hidden_decoder,encoder_outputs_left, encoder_outputs_right,hidden_list_l, hidden_list_r,out_lis_l, out_lis_r):
        ...
        if self.attention_type == 'org':
            a = self.attention.forward_or(hidden_decoder,encoder_outputs)
        else:
            a = self.attention.forward_new(hidden_list_l,hidden_list_r,out_lis_l,out_lis_r)
...
class Dual_adarnn(nn.Module):
    ...
    def for_custom_pre(self, src_left, src_right, trg, teacher_forcing_ratio, len_win=0):
        ...
        ##双向输入且具有分布学习的编码层
        encoder_outputs_left, encoder_outputs_right, out_lis_before, out_lis_after, hidden_lis_before, hidden_lis_after,out_weight_list_before, out_weight_list_after, hidden, hidden_decoder, loss_transfer = \
            self.share_encoder.forward_pre_train(src_left, src_right, len_win)  ##（12，2，128）,hidden(2,2,64)##10，2，16，10，2，16

        ##选取最后一个时间节点的y
        output = src_left[-1, :, 2]  ##选取的是最后一个时间节点的第一个元素

        for t in range(0, max_len):
            output, hidden, attn_weight = self.decoder(output, hidden, hidden_decoder,encoder_outputs_left,
                                                       encoder_outputs_right,hidden_lis_before,hidden_lis_after,
                                                       out_lis_before,out_lis_after)  ##返回的output是[ys,yt],hidden是(1,2,64),a是(2,64)
    ...
    def for_Boosting(self, src_left, src_right, trg, teacher_forcing_ratio=0.5, weight_mat_before=None,
        ...
        ##双向输入且具有分布学习的编码层
        encoder_outputs_left, encoder_outputs_right,out_lis_before, out_lis_after, hidden_lis_before, hidden_lis_after, \
        weight_before, weight_after, hidden, hidden_decoder, loss_transfer, dist_mat_before, dist_mat_after = self.share_encoder.forward_boosting(
    ...
    def predict_ts(self, src_left, src_right, trg, teacher_forcing_ratio=0.5):
        ...
        encoder_outputs_left, encoder_outputs_right,out_lis_before, out_lis_after,hidden_lis_before, hidden_lis_after, hidden, hidden_decoder = self.share_encoder.predict(
            tst_left, tst_right)
